...
import { fetchInBatches, fetchNotionPageBlocks as getPage } from '@/lib/db/notion/getPostBlocks'
import { compressImage, mapImgUrl } from '@/lib/db/notion/mapImage'
import { checkStrIsNotionId, checkStrIsUuid, deepClone } from '@/lib/utils'
...
// Backward compatible aliases used by many pages/components in this repo.
export async function fetchGlobalAllData({ from, locale, pageId } = {}) {
  return await getGlobalData({ pageId: pageId || BLOG.NOTION_PAGE_ID, from, locale })
}

export async function resolvePostProps({ prefix, slug, suffix, locale } = {}) {
  const parts = []
  if (prefix) parts.push(prefix)
  if (slug) parts.push(slug)
  if (suffix) parts.push(...(Array.isArray(suffix) ? suffix : [suffix]))
  let fullSlug = parts.filter(Boolean).join('/')
  const from = `resolve-post-props-${fullSlug || 'root'}`

  const props = await fetchGlobalAllData({ from, locale })
  if (siteConfig('PSEUDO_STATIC', false, props.NOTION_CONFIG)) {
    if (fullSlug && !fullSlug.endsWith('.html')) {
      fullSlug += '.html'
    }
  }

  const matchSlug = fullSlug || prefix || ''
  props.post = props?.allPages?.find(p => {
    if (!p) return false
    if (String(p.type || '').indexOf('Menu') >= 0) return false
    return p.slug === matchSlug || p.slug === matchSlug.replace(/\.html$/, '')
  })

  if (!props?.post && matchSlug && matchSlug.length >= 32) {
    // Fallback: treat slug as pageId when it looks like a notion id/uuid
    const pageId = matchSlug
    const post = await (await import('./notion/getNotionPost')).fetchPageFromNotion(pageId)
    props.post = post
  }
  ...
  const hasValidNotionId = checkStrIsUuid(postId) || checkStrIsNotionId(postId)
  if (hasValidNotionId) {
    try {
      await processPostData(props, from)
    } catch (e) {
      console.error('[resolvePostProps] processPostData failed', e)
      props.prev = null
      props.next = null
