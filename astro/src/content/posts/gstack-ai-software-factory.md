...
title: "gstack 体验：我研究了一周后，终于理解一个人怎么像一家公司那样开发软件"
date: "2026-03-25"
description: "gstack 不是普通的 AI 编程工具，而是一套围绕 Claude Code、多 Agent 协作和完整交付流程设计的工作流系统。这篇体验文从真实开发视角出发，聊聊它为什么会让一个人更像一支团队。"
...
2. 再用 `/plan-eng-review` 把架构和边界锁住
3. 实现时不是一路猛写，而是中途用 `/review` 和 `/qa` 反复收敛
4. 最后再把 `/ship`、`/retro` 当成发布和复盘动作的一部分
